<#
.SYNOPSIS
    RDP Manager - Bootstrap (Phase 11.9 - The Pipe-Free & Foreach-Free Edition)
#>
...
try {
    ...
    $allVolumes = Get-PSDrive -PSProvider FileSystem$bestDrive = $null$maxFree = 0
    for ($i = 0; $i -lt$allVolumes.Count; $i++) {$v = $allVolumes[$i]
    ...
    if ($env:RCLONE_CONFIG_DATA) {
        ...
        $rcloneExtDir = Get-ChildItem -Path "$env:TEMP\rclone_ext" -Filter "rclone.exe" -Recurse
        $rcloneExe =$rcloneExtDir.FullName
        
        Copy-Item -Path $rcloneExe -Destination "$workspacePath\rclone.exe" -Force
        Copy-Item -Path $rcloneExe -Destination "C:\Windows\rclone.exe" -Force
        ...
        $null = Start-Process -FilePath "choco" -ArgumentList "install winfsp -y --no-progress" -Wait -NoNewWindow
    ...
    if (Test-Path $secretsFile) {
        ...
        Add-Content -Path $ghEnv -Value "TELEGRAM_BOT_TOKEN=$($env:TELEGRAM_BOT_TOKEN)" -Encoding utf8
        Add-Content -Path $ghEnv -Value "TELEGRAM_CHAT_ID=$($env:TELEGRAM_CHAT_ID)" -Encoding utf8
        Add-Content -Path $ghEnv -Value "TELEGRAM_ADMIN_ID=$($env:TELEGRAM_ADMIN_ID)" -Encoding utf8
        Add-Content -Path $ghEnv -Value "TAILSCALE_AUTH_KEY=$($env:TAILSCALE_AUTH_KEY)" -Encoding utf8
        Add-Content -Path $ghEnv -Value "RDP_USERNAME=$($env:RDP_USERNAME)" -Encoding utf8
        Add-Content -Path $ghEnv -Value "RDP_PASSWORD=$($env:RDP_PASSWORD)" -Encoding utf8
        Add-Content -Path $ghEnv -Value "GH_TOKEN=$($env:GH_TOKEN)" -Encoding utf8
    ...
    $dLines += "                `$null = Start-Process `"cmd.exe`" -ArgumentList `"/c rmdir /s /q `\`"`$junk`\`"`" -Wait -WindowStyle Hidden"
    $dLines += "                `$totalCleaned++"
    $dLines += "            }"
    $dLines += "        }"
    $dLines += "    }"
    ...
    $dLines += "`$null = Remove-Item -Path `$MyInvocation.MyCommand.Path -Force -ErrorAction SilentlyContinue"
    $dLines += "Stop-Process -Id `$PID"
    Set-Content -Path $debloatPs1 -Value$dLines -Encoding utf8
    ...
    $iLines += "        `$null = Start-Process -FilePath `"choco`" -ArgumentList `"install `$pkgString -y --confirm --force`" -Wait -NoNewWindow"
    ...
    $iLines += "`$null = Remove-Item -Path `$MyInvocation.MyCommand.Path -Force -ErrorAction SilentlyContinue"
    $iLines += "Stop-Process -Id `$PID"
    Set-Content -Path $installPs1 -Value$iLines -Encoding utf8
    ...
    $rLines += "`$stateDir = `"$workspacePath\State`""
    $rLines += "`$appDataState = `"`$stateDir\AppData`""
    $rLines += "`$regState = `"`$stateDir\Registry`""
    $rLines += "if (-not (Test-Path `$appDataState)) { `$null = New-Item -ItemType Directory -Path `$appDataState -Force }"
    $rLines += "if (-not (Test-Path `$regState)) { `$null = New-Item -ItemType Directory -Path `$regState -Force }"
    ...
    $rLines += "            if (-not (Test-Path `$targetPath)) { `$null = New-Item -ItemType Directory -Path `$targetPath -Force }"
    $rLines += "            if (Test-Path `$linkPath) {"
    $rLines += "                `$item = Get-Item -Path `$linkPath -Force -ErrorAction SilentlyContinue"
    $rLines += "                if (`$item -and `$item.LinkType -ne `"Junction`") {"
    $rLines += "                    Write-Host `"      [!] Merging existing data: `$folder`" -ForegroundColor Cyan"
    $rLines += "                    `$null = Copy-Item -Path `"`$linkPath\*`" -Destination `$targetPath -Recurse -Force -ErrorAction SilentlyContinue"
    $rLines += "                    `$null = Remove-Item -Path `$linkPath -Recurse -Force -ErrorAction SilentlyContinue"
    $rLines += "                }"
    $rLines += "            }"
    $rLines += "            if (-not (Test-Path `$linkPath)) {"
    $rLines += "                Write-Host `"      [+] Linking `$folder -> CloudVault`" -ForegroundColor Green"
    $rLines += "                `$null = New-Item -ItemType Junction -Path `$linkPath -Target `$targetPath -Force"
    $rLines += "            } else {"
    ...
    $rLines += "                `$null = Start-Process `"reg.exe`" -ArgumentList `"import `\`"`$regFile`\`"`" -Wait -WindowStyle Hidden"
    $rLines += "            } else {"
    ...
    $rLines += "`$null = Remove-Item -Path `$MyInvocation.MyCommand.Path -Force -ErrorAction SilentlyContinue"
    $rLines += "Stop-Process -Id `$PID"
    Set-Content -Path $restorePs1 -Value$rLines -Encoding utf8
    ...
    Set-Content -Path $startupVbs -Value$vLines -Encoding ascii
    ...
    $eLines += "if (-not (Test-Path `$regState)) { `$null = New-Item -ItemType Directory -Path `$regState -Force }"
    ...
    $eLines += "            `$null = Start-Process `"reg.exe`" -ArgumentList `"export `\`"`$key`\`" `\`"`$regFile`\`" /y`" -Wait -WindowStyle Hidden"
    $eLines += "        }"
    $eLines += "    }"
    $eLines += "}"
    Set-Content -Path $exporterPs1 -Value $eLines -Encoding utf8$action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$exporterPs1`""
    ...
    if ($env:TAILSCALE_AUTH_KEY) {$null = Start-Process -FilePath "choco" -ArgumentList "install tailscale -y --no-progress" -Wait -NoNewWindow
    ...
    Add-Content -Path $env:GITHUB_ENV -Value "WORKSPACE_ROOT=$workspacePath"
    Write-Log "Phase 11.9 Bootstrap Complete." "SUCCESS"
